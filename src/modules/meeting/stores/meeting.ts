...
export const useMeetingStore = defineStore('meeting', () => {
  ...
  const { xPortalDevice, meetingId, accessToken, callAccount } = storeToRefs(authStore);
  ...
  function startUserAgent(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        const socket: WebSocketInterface = new JsSIP.WebSocketInterface(appConfig.call.host);
  ...
  async function makeCall(
    ...
    try {
      ...
      const callOptions = {
        ...
        extraHeaders: [`X-Webitel-Meeting: ${meetingId.value}`],
